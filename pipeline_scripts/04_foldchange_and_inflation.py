...
def main():
    parser = argparse.ArgumentParser(
        description="Step 2 - Block 5: Log2 Fold Change Comparison Pipeline (Unique vs LCA vs Mixture Model)"
    )
    parser.add_argument(
        "--input_tsv",
    ...
    if not input_path.exists():
        # Fallback if in testing environment and file doesn't exist
    ...
    # ==========================================================================
    # METHOD 3: MIXTURE MODEL LOG2 FC (Expectation-Maximization)
    # ==========================================================================
    ...
    for node, descendants in node_to_desc.items():
        ...
        for num_cond, den_cond, num_s, den_s in comparisons:
            ...
            if exp_num_ug == 0 and exp_den_ug == 0:
                continue

            expected_log2fc = np.log2((exp_num_ug + eps) / (exp_den_ug + eps))
    ...
    if eval_df.empty:
        eval_df = pd.DataFrame(columns=[
            "Node", "Node_Type", "Num_Species", "Comparison",
            "Expected_Log2FC", "Unique_Log2FC", "Unique_Excluded",
            "LCA_Log2FC", "LCA_Excluded", "Mix_Log2FC", "Mix_Excluded"
        ])
    ...
    methods = [
        ("Unique_Log2FC", "Unique_Excluded", "Unique Peptides Only (Discard Shared)", "Blues"),
        ("LCA_Log2FC", "LCA_Excluded", "Rigid LCA Assignment (Paso 2)", "Oranges"),
        ("Mix_Log2FC", "Mix_Excluded", "Mixture Model (Proportional Split)", "Greens")
    ]
    ...
    for row_idx, comp_key in enumerate(comp_keys):
        ...
        max_depth = comp_df["Num_Species"].max()
        ...
        for col_idx, (col_name, excl_name, method_title, cmap_base) in enumerate(methods):
            ...
            # Calculate linear correlations (no log needed since these are already log2 values!)
            ...
            # Identity line (FC can be negative, so we span from -15 to 15)
            min_lim = -15.0
            max_lim = 15.0
            ...
            for _, row in plot_df.iterrows():
                ...
                if is_leaf:
                    ...
                    # or if they are close to the main data cloud to avoid clutter
                    if abs(row["Expected_Log2FC"]) > 1.0:
                        short_ancestor = name
    ...
    plt.suptitle("METAPROTEOMICS LOG2 FOLD CHANGE VALIDATION BENCHMARK MATRIX\n"
                 "Comparison of unique peptides only, strict LCA, and proportional Mixture Model across all pairs",
                 fontsize=16, fontweight="bold", y=0.99)
    plt.tight_layout()

    plot_path = outdir / "step2_mixture_model_foldchange_matrix.png"
    plt.savefig(plot_path, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"[SUCCESS] 3x3 Log2 FC Benchmark plot matrix saved to: {plot_path}")

    # ==========================================================================
    # GENERATE INFLATION VS NODE DEPTH PLOT
    # ==========================================================================
    print("Generating Inflation vs Node Depth plot...")

    # Calculate inflation (absolute deviation from expected)
    eval_df["Unique_Inflation"] = abs(eval_df["Unique_Log2FC"] - eval_df["Expected_Log2FC"])
    eval_df["LCA_Inflation"] = abs(eval_df["LCA_Log2FC"] - eval_df["Expected_Log2FC"])
    eval_df["Mix_Inflation"] = abs(eval_df["Mix_Log2FC"] - eval_df["Expected_Log2FC"])

    fig, axs = plt.subplots(1, 3, figsize=(18, 6), sharey=True)
    fig.suptitle("Inflation (Absolute Error in Log2FC) vs Node Depth", fontsize=16, fontweight="bold")

    inflation_methods = [
        ("Unique_Inflation", "Unique_Excluded", "Unique Only", "Blues"),
        ("LCA_Inflation", "LCA_Excluded", "Rigid LCA", "Oranges"),
        ("Mix_Inflation", "Mix_Excluded", "EM Mixture Model", "Greens")
    ]

    for col_idx, (infl_col, excl_col, title, cmap) in enumerate(inflation_methods):
        ax = axs[col_idx]

        valid_df = eval_df[~eval_df[excl_col] & eval_df[infl_col].notna()].copy()
        if valid_df.empty:
            ax.set_title(f"No Data\n{title}")
            continue

        # Add slight jitter to X for visualization
        jitter = np.random.normal(0, 0.1, size=len(valid_df))

        sc = ax.scatter(
            valid_df["Num_Species"] + jitter,
            valid_df[infl_col],
            c=valid_df["Num_Species"], cmap=cmap, alpha=0.7, edgecolors='k', s=80
        )

        # Calculate trendline (Linear regression of Inflation vs Depth)
        x = valid_df["Num_Species"]
        y = valid_df[infl_col]
        if len(x) > 1:
            z = np.polyfit(x, y, 1)
            p = np.poly1d(z)
        ...
        ax.set_xlabel("Node Depth (Number of Descendant Species)", fontsize=10)
        if col_idx == 0:
            ax.set_ylabel("Inflation (|Measured - Expected Log2FC|)", fontsize=10)
        ax.grid(True, linestyle="--", alpha=0.4)

        plt.colorbar(sc, ax=ax, pad=0.02, label="Descendant Species Count")

    plt.tight_layout()
    inflation_path = outdir / "step2_mixture_model_inflation_vs_depth.png"
    plt.savefig(inflation_path, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"[SUCCESS] Inflation vs Depth plot saved to: {inflation_path}")
    ...
    # ==========================================================================
    print("\n" + "="*95)
    print("           METAPROTEOMICS LOG2 FOLD CHANGE METHODOLOGY COMPARATIVE BENCHMARK")
    print("="*95)
    for comp_key in comp_keys:
        ...
        for col_name, excl_name, title, _ in methods:
            ...
            # Pearson & Spearman (normal scale because Log2 values are already on a log scale!)
